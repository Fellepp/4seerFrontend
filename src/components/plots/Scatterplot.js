...
const ConnectedScatterplot = ({ columns, inputs, changeInputs, states, changeState, img, changeImage }) => {
    ...
    return (
        <div>
            ...
            <select id="x_label" onChange={handleChange}>
                {loColumns.map((column) => (
                    <option key={column} label={column}></option>
                ))}
            </select>X-label<br/>

            <select id="y_label" placeholder="Hei" onChange={handleChange}>
                {loColumns.map((column) => (
                    <option key={column} label={column}></option>
                ))}
            </select>Y-label<br/>
...
const Scatterplot = connect(mapStateToProps, mapDispatchToProps)(ConnectedScatterplot)
